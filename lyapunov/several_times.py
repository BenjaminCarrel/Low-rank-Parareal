...
for k in range(len(all_t_span)):
    ...
    full_iv = PB.Y0.full()
    reference_solution = classical_solvers.solve(PB, all_t_span[k], full_iv, nb_t_steps=nb_t_steps, monitoring=True, method='scipy')

    # COMPUTE LOW-RANK PARAREAL
    ts, Unk, _, _ = parareal.Low_Rank_Parareal(PB, all_t_span[k], PB.Y0, nb_t_steps, coarse_rank=4, fine_rank=16,
                                         nb_coarse_substeps=80, nb_fine_substeps=80, DLRA_method='KSL2')
    Enk = parareal.compute_parareal_error(reference_solution, Unk)
    max_Enk = np.max(Enk, axis=0)

    # PLOT
    plt.semilogy(it, max_Enk, linestyles[k], label=f'T = {round(all_t_span[k][1]-0.01, 2)}')
...
plt.legend()
plt.show()
fig.savefig('figures/lyapunov_several_times.pdf')
# %%
