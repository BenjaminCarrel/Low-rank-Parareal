...
anim_sing_vals = plotting.singular_values(reference_sol.ts, reference_sol.Ys, title='figures/animation_riccati_sing_vals.gif', do_save=True)
...
for i in i_times:
    ...
    plt.title(f'Singular value at time t={round(t-0.01,2)}', fontsize=16)
    plt.xlim((1,50))
    ...
    plt.grid()
    plt.show()
    fig.savefig(f'figures/riccati_sing_vals_t_{i}.pdf')
...
for i in i_times:
    ...
    x = np.linspace(0,1,m)
    X, Y = np.meshgrid(x, x)
    fig = plotting.plot_3D(X, Y, reference_sol.Ys[i], title=f'Solution at time t={round(t-0.01, 2)}')
    fig.savefig(f'figures/riccati_3D_solution_t_{i}.pdf')
# %%
